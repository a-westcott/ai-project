...
class Board:
    ...
    def get_actions(self, board, x, y):
        ...
        for destination in all_coords:
            x1,y1 = destination
            if board[x1][y1] == 0:
                continue
            dest_colour = BLACK if board[x1][y1] < 0 else WHITE
                
            if colour != dest_colour:
                all_coords.remove(destination)
            ...
            for x in range(8):
                for y in range(8):
                    if board[x][y] == 0:
                        ...
                        tmp, n_explosions = explode(tmp, x, y)


    def explode(self, board, x, y, n_explosions=0):
        ...
        for x,y in radius:
            ...
            if board[x][y]:
                board[x][y] = 0
                n_explosions += 1
                board, n_explosions = self.explode(board, x, y, n_explosions)
